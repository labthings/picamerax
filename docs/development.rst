...
:mod:`~picamerax.mmalobj` layer. This is a layer introduced in picamerax 1.11 to
ease the usage of ``libmmal`` (the underlying library that picamerax,
``raspistill``, and ``raspivid`` all rely upon).

Beneath :mod:`~picamerax.mmalobj` is a :mod:`ctypes` translation of the
...
necessary to hack on picamerax).
...
If you wish to develop picamerax itself, it is easiest to obtain the source by
...
If you wish to run the picamerax test suite, follow the instructions in
