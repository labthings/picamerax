...
.. module:: picamerax.mmalobj

.. currentmodule:: picamerax.mmalobj

This module provides an object-oriented interface to ``libmmal`` which is the
library underlying picamerax, ``raspistill``, and ``raspivid``.  It is provided
...
* A port (:class:`MMALControlPort` and derivatives) is either a control port,
  ...
  - Control ports are used to accept and receive commands, configuration
    ...
    but in picamerax they're only used for component configuration.
  ...
  - Input and output ports can be audio, video or sub-picture (subtitle) ports,
    but picamerax only deals with video ports.
...
We'll start by importing the :mod:`~picamerax.mmalobj` module with a convenient
...
have a look at the source for the :class:`~picamerax.PiCamera` class, especially
...
of how picamerax operates under the hood.

The major difference between picamerax and a "typical" MMAL setup is that upon
construction, the :class:`~picamerax.PiCamera` class constructs both a
...
:meth:`~picamerax.PiCamera.capture`, :meth:`~picamerax.PiCamera.start_recording`,
etc. The following illustrates a typical picamerax pipeline whilst video
recording without a preview:

.. image:: images/picamerax_pipeline.*
    :align: center
...
in picamerax includes the concept of a "Python MMAL" component. To the user
...
subclass :class:`picamerax.array.PiArrayTransform`. This derives from
...
picamerax library. Users of ``mmalobj`` may find them handy in various
situations:
